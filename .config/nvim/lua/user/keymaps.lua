...
map("n", "<C-A-k>", "<C-w>+", { desc = "Increase split height" })
map("n", "<C-A-j>", "<C-w>-", { desc = "Decrease split height" })
map("n", "<C-A-l>", "<C-w>>", { desc = "Increase split width" })
map("n", "<C-A-h>", "<C-w><", { desc = "Decrease split width" })
...
map(
  ...
  "<c-s>",
  "y:%s/\\v<c-r>=escape(@\", '/\\.*$^~[')<CR>/",
  { desc = "Search and replace selection in whole file" }
)
