...
vim.schedule(function()
  ...
  M.rename_file = function(opts)
    ...
    local function rename()
      ...
      M.on_rename_file(from, to, function()
        ...
        if opts.on_rename then opts.on_rename(to, from, ok) end
      end)
    end
    if to then return rename() end
  ...
  function M._rename(from, to)
    ...
    if not vim.api.nvim_buf_is_valid(from_buf) then return false end
    local to_buf = vim.fn.bufnr(to)
    if vim.api.nvim_buf_is_valid(to_buf) then vim.api.nvim_buf_delete(to_buf, { force = true }) end
